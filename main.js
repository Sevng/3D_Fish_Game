...
class BasicWorldDemo {
  ...
  _Initialize() {
    ...
    const fov = 10;         //(10) 카메라 깊이 (+ 멀어짐) 
    const aspect = 1920 / 1080;
    const near = 1.0;      
    const far = 20000.0;
	
	this.camera_ = new THREE.PerspectiveCamera(fov, aspect, near, far);
    this.camera_.position.set(-150 , 20, 50);   // 카메라시점 (100 (- 우회전), 50(+ 위에서보기, - 아래서보기), (100)완전 2D) 
    this.camera_.lookAt(10, 3, 0); 
	
    this.scene_ = new THREE.Scene();

    let light = new THREE.DirectionalLight(0xFFFFFF, 1.0);
    light.position.set(100, 125, 0);
    ...
    this.scene_.fog = new THREE.FogExp2(0x89b2eb, 0.00325);

    ////////////////// 바닥
    const textureLoader = new THREE.TextureLoader();
    const textureBaseColor = textureLoader.load('resources/sand/sand_basecolor.png');
	const texturenormalmap = textureLoader.load('resources/sand/sand_normal.png');
	const textureHeightMap = textureLoader.load('resources/sand/sand_height.png');

    const ground = new THREE.Mesh(
        new THREE.PlaneGeometry(1500, 600, 10, 10), // 땅의 크기 
    ...
    ground.rotation.x = -Math.PI / 2; // 바닥의 각도
	ground.position.set(10, -5, 1);   // 바닥의 위치 
    this.scene_.add(ground);
    
	
    const uniforms = {  
      topColor: { value: new THREE.Color(0x0077FF) },   ///하늘 색 
